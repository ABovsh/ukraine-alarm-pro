...
Картка входить до інтеграції: вона сама додається в **Налаштування → Панелі → Ресурси**,
окремо встановлювати нічого не треба. Для панелей у режимі YAML ресурс додати вручну:
`/ukraine_alarm_pro/ukraine-alarm-pro-card.js`, тип `module`. Після встановлення
чи оновлення перезапустіть Home Assistant і оновіть сторінку браузера. Далі: панель →
**Редагувати** → **Додати картку** → знайдіть **Ukraine Alarm Pro**. Якщо регіон один,
картка знайде його сама; якщо кілька — оберіть сенсор тривоги потрібного регіону.

Картка показує стан тривоги, тип загрози, рівень, скільки триває тривога і з якого часу,
охоплення, причину, останню подію й актуальність даних. Застарілі дані позначаються як
«Немає свіжих даних», а не як «Тихо». Тривалість рахує браузер, тож картка не створює
записів у базі. Сутності можна перейменовувати: картка знаходить їх за регіоном.
