...
![Картка Ukraine Alarm Pro](docs/images/status-card.png)

Одна картка показує все, що інтеграція знає про регіон:

- **Стан.** Тривога або «Тихо», тип загрози, рівень (жовтий чи червоний), причина й
  охоплення: увесь регіон чи його частина з переліком районів або громад. Коли дані
  застаріли, картка пише «Немає свіжих даних», а не «Тихо».
- **Час.** Під час тривоги — скільки вона триває і з котрої години. Без тривоги — скільки
  часу минуло від останнього відбою.
- **24 години.** Кількість тривог, їх загальна тривалість і частка часу під тривогою.
  Під цим рядком — смуга доби: червоні й жовті відрізки позначають тривоги. Навівши
  курсор на відрізок, видно його початок, кінець і тривалість.
- **7 днів.** Кількість тривог, загальна тривалість і частка часу під тривогою, а також
  найдовша й середня тривога.
- **Дані.** Чи актуальні дані й коли вони оновилися.

Натискання на картку відкриває сенсор тривоги з його історією.

Компактний вигляд (`layout: compact`) займає один рядок: назва регіону, стан і час від останнього відбою.

![Компактний вигляд картки Ukraine Alarm Pro](docs/images/status-card-compact.jpg)

### Як додати картку

1. Встановіть інтеграцію й перезапустіть Home Assistant. Картка сама додається в
   **Налаштування → Панелі → Ресурси**, окремо встановлювати нічого не треба.
2. Оновіть сторінку браузера. У застосунку Home Assistant: **Налаштування → Companion
   app → Troubleshooting → Reset frontend cache**, потім перезапустіть застосунок.
3. Відкрийте панель → олівець **Редагувати** → **Додати картку**.
4. У пошуку введіть **Ukraine Alarm Pro** і оберіть картку.
5. Якщо регіон один, картка знайде його сама — натисніть **Зберегти**. Якщо регіонів
   кілька, у полі **Region / Регіон** оберіть сенсор тривоги потрібного регіону
   («…: тривога»). Для кожного регіону додайте окрему картку.

У редакторі картки можна змінити назву (наприклад, «Дім» замість назви громади),
обрати вигляд — повний (стан і статистика), лише стан або компактний в один рядок — і мову.
...
режимі YAML, ресурс треба додати вручну: `/ukraine_alarm_pro/ukraine-alarm-pro-card.js`,
тип `module`.
...
layout: full                        # full — стан і статистика, status — лише стан, compact — один рядок
