...
Приклад панелі на стандартних картках — [`docs/examples/dashboard.yaml`](docs/examples/dashboard.yaml).
Вона показує стан тривоги, типи загроз, рівень, час оголошення й тривалість, охоплення,
причини, останню подію та актуальність даних. Застарілі дані з останнім станом «тихо»
позначаються як «немає свіжих даних», а не як підтверджена тиша. Замініть `31` на ID
свого регіону та вставте YAML у редактор сирої конфігурації панелі.

Якщо потрібна лише одна картка, візьміть [`docs/examples/status-card.yaml`](docs/examples/status-card.yaml):
один рядок на регіон, наприклад «🚨 **м. Київ** · повітряна 🟡 · з 09:38, 6 хв» або
«🟢 **м. Київ** · тихо».
